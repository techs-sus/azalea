...
/*
	...
	13 is a "carriage return" character
	34 is a "double quote" character
	38 is a "ampersand" character
	92 is a "backslash" character
*/

/// The "Base123" set is optimal for usage with a Lua double quoted string.
/// It excludes the ampersand from Base122 because ampersands are legal in Lua strings.
...
mod tests {
	...
	/// Example:
	/// ```ignore
	/// generate_base122_test!(&[], &[], empty_vectors_alike);
	/// ```
	macro_rules! generate_base122_test {
		($original:expr, $encoded:expr, $name:ident) => {
			#[test]
			fn $name() {
				assert_eq!(base122_encode($original), $encoded);
			}
		};
	}
	...
	/// Example:
	/// ```ignore
	/// generate_base123_test!(&[], &[], empty_vectors_alike);
	/// ```
	macro_rules! generate_base123_test {
		($original:expr, $encoded:expr, $name:ident) => {
			#[test]
			fn $name() {
				assert_eq!(base123_encode($original), $encoded);
			}
		};
	}
	...
	mod base123 {
		...
		generate_base123_test!(
			b"hello world",
			&[52, 25, 45, 70, 99, 60, 64, 119, 55, 211, 141, 70, 32],
			encode_hello_world
		);
		...
		generate_base123_test!(&[0, 0], &[194, 128, 222, 128], encode_two_zeros);
		generate_base123_test!(&[0, 0, 0], &[194, 128, 194, 128], encode_three_zeros);
		generate_base123_test!(
			&[0, 0, 0, 0],
			&[194, 128, 194, 128, 222, 128],
			encode_four_zeros
		);
		generate_base123_test!(
			&[0, 0, 0, 0, 0],
			&[194, 128, 194, 128, 194, 128],
			encode_five_zeros
		);

		generate_base123_test!(
			&BASE123_ILLEGAL_BYTES,
			&[194, 130, 65, 82, 18, 112],
		...
		generate_base123_test!(
			...
			&[
				194, 128, 32, 32, 24, 16, 198, 134, 3, 66, 1, 16, 80, 44, 24, 202, 135, 3, 98, 1, 8, 72,
				38, 20, 199, 133, 66, 113, 64, 100, 52, 27, 14, 7, 35, 97, 121, 195, 130, 206, 145, 73, 4,
				82, 49, 28, 80, 41, 21, 199, 165, 66, 105, 56, 94, 48, 24, 76, 70, 51, 33, 84, 108, 55, 28,
				14, 39, 35, 89, 112, 122, 62, 31, 80, 8, 20, 18, 202, 136, 69, 35, 17, 105, 4, 74, 41, 22,
				76, 38, 83, 73, 117, 2, 69, 36, 83, 42, 21, 42, 101, 58, 97, 50, 90, 45, 87, 11, 85, 114,
				125, 64, 97, 49, 24, 108, 70, 43, 25, 78, 104, 52, 90, 77, 54, 99, 53, 211, 175, 56, 28,
				46, 39, 27, 81, 106, 118, 59, 94, 15, 23, 83, 109, 120, 125, 63, 31, 112, 8, 12, 198, 135,
				4, 66, 97, 80, 120, 68, 38, 21, 11, 70, 35, 49, 104, 124, 66, 35, 18, 73, 101, 18, 89, 52,
				94, 49, 25, 77, 38, 115, 73, 108, 122, 63, 32, 80, 104, 84, 58, 37, 22, 77, 39, 84, 42, 53,
				42, 93, 50, 91, 46, 87, 108, 22, 27, 21, 78, 105, 53, 91, 45, 119, 11, 77, 106, 119, 60,
				94, 111, 87, 124, 6, 7, 5, 67, 98, 49, 56, 108, 62, 35, 19, 74, 101, 115, 25, 211, 182, 63,
				33, 81, 105, 52, 122, 77, 46, 91, 47, 88, 108, 118, 91, 61, 102, 119, 61, 95, 112, 56, 60,
				46, 31, 19, 75, 102, 115, 122, 29, 30, 87, 47, 89, 109, 119, 59, 126, 15, 15, 75, 103, 116,
				122, 125, 94, 127, 71, 103, 117, 123, 126, 63, 63, 111, 120
			],
	...
	mod base122 {
		...
		/* https://github.com/vence722/base122-go/blob/43ce4390d9cf093ef6dd7e225792390089f8fe9f/encoder_test.go#L38-L44 */
		generate_base122_test!(b"very very very very very very very very very very very very very very very very long text!!!",
		&[
			59, 25, 46, 39, 73, 1, 108, 101, 57, 30, 36, 7, 51, 21, 100, 121, 16, 29, 76, 87, 19, 100,
			64, 118, 50, 215, 143, 18, 3, 89, 74, 114, 60, 72, 14, 102, 43, 73, 114, 32, 59, 25, 46, 39,
			73, 1, 108, 101, 57, 30, 36, 7, 51, 21, 100, 121, 16, 29, 76, 87, 19, 100, 64, 118, 50, 215,
			143, 18, 3, 89, 74, 114, 60, 72, 14, 102, 43, 73, 114, 32, 59, 25, 46, 39, 73, 1, 108, 101,
			57, 30, 36, 6, 99, 61, 215, 167, 16, 29, 12, 87, 67, 80, 66, 33, 16, 64
		],
		encode_very_long_text
	);
	}
